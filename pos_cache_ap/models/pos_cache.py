...
class pos_cache(models.Model):
    ...
    def refresh_cache(self):
        ...
        for data in self.env['product.product'].search([('is_updated','=',True)]):
            data.is_updated = False
        for data in self.env['product.product'].search([('is_new','=',True)]):
            data.is_new = False
        self.write(datas)
...
class pos_config(models.Model):
    ...
    def get_products_from_cache(self, fields, domain):
        ...
        print 'cache_for_user...............',cache_for_user
        if cache_for_user:
            catche_return = cache_for_user.get_cache(domain, fields)
            print type(catche_return)
            products_ids = [data.id for data in self.env['product.product'].search([('is_updated','=',True)])]
            products_new_ids = self.env['product.product'].search([('is_new','=',True)])
            prod_ctx = products_new_ids.with_context(pricelist=cache_for_user.config_id.pricelist_id.id, display_default_code=False,
                                lang=cache_for_user.compute_user_id.lang)
            res = prod_ctx.read(cache_for_user.get_product_fields())
            print res
            for new_prod in res:
                catche_return.append(new_prod)
            if products_ids:
                for rec in catche_return:
                    if rec['id'] in products_ids:
                        product = self.env['product.product'].search([('id','=',rec['id'])])
                        rec['list_price'] = product.list_price
                        rec['price'] = product.list_price
            return catche_return
        else:
            pos_cache = self.env['pos.cache']
            pos_cache.create({
                'config_id': self.id,
