...
#                                       ('type','in',('out_invoice','out_refund')),
        ...
        for mvl in moveln_ids:
            ...
            due_days = (datetime.strptime(statment_date, '%Y-%m-%d') - date_due).days + 1
        ...
        for inv in all_invoices:
            ...
            date_due = datetime.strptime(inv['date_invoice'], '%Y-%m-%d')
            due_days = (datetime.strptime(statment_date, '%Y-%m-%d') - date_due).days + 1
            print due_days
            #total_amount = inv.amount_total
            total_amount = inv.residual
...
#                               })
    ...
    def render_html(self, docids, data=None):
        docs = self.env['res.partner'].browse(data.get('active_ids_ap'))
        docargs = {
            'doc_ids': data.get('active_ids_ap'),
